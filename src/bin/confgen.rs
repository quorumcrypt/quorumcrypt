...
use quorumcrypt_blockchain_stub::cli::types::{P2PConfig, PeerP2PInfo};
...
fn main() {
    ...
    let ips = match network_targets_from_file(&confgen_cli.ip_file, true) {
        Ok(ips) => ips,
        Err(e) => {
            error!("{}", e);
            exit(1);
    ...
    if let Some(path) = confgen_cli.integration_file {
        // For proxy nodes, we allow connecting via DNS.
        ips_proxy_nodes = match network_targets_from_file(&path, false) {
            Ok(ips) => Some(ips),
            Err(e) => {
                error!("{}", e);
                exit(1);
...
// Read a set of network targets from a file, one per line.
// 
// The second parameter, if set to true, requires the targets to be valid IP (either v4 or v6)
// addresses, disallowing the use of e.g. DNS names, or local host aliases.
fn network_targets_from_file(path: &PathBuf, require_ip: bool) -> Result<Vec<String>, String> {
    ...
    if !require_ip {
        return Ok(lines.map(String::from).collect());
    }
