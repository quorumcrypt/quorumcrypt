// Tokio
use log::{error, info};
use quorum_proto::proxy_api::proxy_api_client::ProxyApiClient;
...
fn forward_to_all(config: P2PConfig, message: &[u8]) -> Result<(), String> {
    ...
    for peer in config.peers.iter() {
        ...
        tokio::spawn(async move {
            ...
            info!(
                "[BlockchainStub] Received request and forwarded it to the other Quorumcrypt nodes!"
            );
        });
    }
...
impl ProxyApi for QuorumcryptBlockchainStub {
...
impl QuorumcryptBlockchainStub {

}

async fn start_and_run(service: QuorumcryptBlockchainStub, address: SocketAddr) -> io::Result<()> {
...
async fn main() -> io::Result<()> {
    ...
    let service = QuorumcryptBlockchainStub {
