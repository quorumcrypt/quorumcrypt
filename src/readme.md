# Quorumcrypt - Threshold Cryptography in Rust

Quorumcrypt architecture encompasses three layers:
...
Alternatively, one can use Quorumcrypt as a library through the **SchemeAPI** and directly access the cryptographic primitives in a more fine-grained manner.
<!-- 
![Architectural Overview](img/Quorumcrypt-stack.png) -->
<!-- Using HTML to enforce size -->
<p align="center">
<img src="img/Quorumcrypt-stack.png" alt="Architectural Overview" width="600">
</p>

## Repo organization

The current directory contains all the code developed for the codebase of Quorumcrypt. The modularity is reflected in the `Cargo.toml` file in the root directory. The root package `quorumcrypt` provides all the binary necessary to configure, start and execute the service. Then aggregates in the same workspace the packages `service`, `core` (with its inner structure), and `network` to provide an exact mapping to the architecture presented. The directory `proto` contains the specification of the protobuf types used to implement the API of the service with gRPC. The `conf` directory acts as a staging area for configuraton files, created by the binaries for the setup of the service, and used by instances of the servers. 
...
To try immediately Quorumcrypt go back to the repo's root folder and checkout the demo with Docker.
...
To run a local Quorumcrypt network one can use the `setup.sh` script, which automates all configuration steps needed. 
Under `src` one can run: 
```
sh setup.sh <N_servers>
```
The output will be a second script `start_network.sh`, which will allow running a Quorumcrypt network consisting of <N_servers>. 
```
sh start_network.sh
```
...
The codebase of Quorumcrypt provides a binary, `QuorumCLI`, to perform complementary tasks. Said binary can be used with the parameter `keygen` to perform the initial setup and key generation and distribution among a set of `N` servers.
It writes the keys for each server in a chosen directory. For a deployment with 4 servers and a threshold of 3, run:
```
cargo run --bin quorumcli -- keygen -k=3 -n=4 --subjects Sg02-Bls12381 --output ./conf
```
...
```
cargo run --bin quorumcli -- keygen -k=3 -n=4 --subjects Sg02-Bls12381 --output ./conf --new
```
...
To setup the network of servers with all possible available schemes you can run `quorumcli` script with the `--subjects` flag set to "*all*":
```
cargo run --bin quorumcli -- keygen -k=3 -n=4 --subjects all --output ./conf --new
```
...
## Run the binary QuorumCLI for complementary tasks

There exists a CLI application which can be used to encrypt files and generate keys. Use `cargo run --bin quorumcli` to build and run the CLI application. 

We already used this application for **generating the keys**, but it has other two functions. It can be used to **encrypt** a piece of data that later might be decrypted by the servers in the Quorumcrypt network, and to **verify** a signature issued by the network.

To explore the other functions, run: 
```
cargo run --bin quorumcli -- --help
```
to receive the help prompt for specific subcommands, run: 
```
cargo run --bin quorumcli -- <SUBCOMMAND> --help
```


<!-- Usage: `./quorumcli [action] [params]`
available actions:
...
- `keygen -k [k] -n [n] --subjects [subjects] --output [output_dir]` \
  ...
  example: `./quorumcli keygen 3 5 sg02-bls12381,bz03-ed25519 /path/to/keys/` <br>
...
- `keystore [action] [keystore_location]` \
    ...
    &emsp; `--address` specifies the remote Quorumcrypt node to connect to (format `https://ip:port`) \
